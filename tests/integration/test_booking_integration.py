...
def test_from_booking_to_welcome(client):
    response = client.get(
        f"/book/going/book",
    ...
    booking = client.post("/purchasePlaces",
        data={"competition": "empty", "club": "purchase_places", "places": "1"},
        follow_redirects=True,
    )
    decoded_response = decode_response(booking.data)

    assert html_checks.is_club_name_displayed_in_welcome_html(decoded_response, "purchase_places")
