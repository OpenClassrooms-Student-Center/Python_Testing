...
def is_log_in_form_in_html(parsed_html):
...
def are_points_displayed_in_welcome_html(parsed_html):
    return "Points available: " in parsed_html


def is_display_board_link_available_in_welcome_html(parsed_html):
    return DISPLAY_BOARD_LINK in parsed_html


def is_competition_list_displayed_in_welcome_html(parsed_html):
    for html_snippets in COMPETITIONS_LIST_TEMPLATE:
        assert html_snippets in parsed_html
    return True


def are_competition_details_displayed_in_welcome_html(parsed_html):
    for html_snippets in COMPETITION_DETAILS_TEMPLATE:
        assert html_snippets in parsed_html
    return True


def is_book_option_available_for_going_competition_in_welcome_html(parsed_html):
    return '<a href="/book/going/show_summary">Book Places</a>' in parsed_html


def is_book_option_unavailable_for_over_competition_in_welcome_html(parsed_html):
    return '<a href="/book/over/show_summary">Book Places</a>' not in parsed_html


def is_book_option_unavailable_for_full_competition_in_welcome_html(parsed_html):
    return '<a href="/book/full/show_summary">Book Places</a>' not in parsed_html


def is_competition_name_displayed_in_booking_html(parsed_html):
    return "<title>Booking for" and "|| GUDLFT</title>" in parsed_html


def are_competition_places_displayed_in_booking_html(parsed_html):
    return "Places available:" in parsed_html


def is_competition_date_displayed_in_booking_html(parsed_html):
    return "This competition is open until" in parsed_html


def is_booking_form_displayed_in_booking_html(parsed_html):
    for html_snippets in BOOK_COMPETITION_FORM:
        assert html_snippets in parsed_html
    return True


def are_clubs_points_displayed_in_display_board_html(parsed_html):
    return "remaining points:" in parsed_html


def is_club_list_displayed_in_display_board_html(parsed_html):
